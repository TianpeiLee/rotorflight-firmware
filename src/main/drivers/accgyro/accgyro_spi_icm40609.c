...
#define ICM40609_ACCEL_FS_SEL_16G           (0 << 5) //这个和手册不符，手册为32G,sacl为1024
...
// ACCEL_CONFIG0_REG - 0x50 bits [7:5]
#define ICM40608_ACCEL_FS_SEL_16G           (0 << 5) //default
#define ICM40608_ACCEL_FS_SEL_8G            (1 << 5)
#define ICM40608_ACCEL_FS_SEL_4G            (2 << 5)
#define ICM40608_ACCEL_FS_SEL_2G            (3 << 5)
#define ICM40608_ACCEL_FS_SEL_1G            (4 << 5)
#define ICM40608_ACCEL_FS_SEL_0_5G          (5 << 5)
#define ICM40608_ACCEL_FS_SEL_0_25G         (6 << 5)
#define ICM40608_ACCEL_FS_SEL_0_125G        (7 << 5)
...
static const ICM40609_AafProfile aafProfiles_ICM40608[ICM40609_AAF_PROFILE_COUNT] = {
    { 10  , 1, 1, 15 },
    { 21  , 2, 4, 13 },
    { 32  , 3, 9, 12 },
    { 42 , 4, 16, 11 },
...
typedef enum {
    ICM40609_GYRO_NF_BW_1449HZ = 0,     //ICM40608_GYRO_NF_BW_362HZ
    ICM40609_GYRO_NF_BW_680HZ  = 1,     //ICM40608_GYRO_NF_BW_170HZ
    ICM40609_GYRO_NF_BW_329HZ  = 2,     //ICM40608_GYRO_NF_BW_83HZ
    ICM40609_GYRO_NF_BW_162HZ  = 3,     //ICM40608_GYRO_NF_BW_41HZ
    ICM40609_GYRO_NF_BW_80HZ   = 4,     //ICM40608_GYRO_NF_BW_21HZ
    ICM40609_GYRO_NF_BW_40HZ   = 5,     //ICM40608_GYRO_NF_BW_11HZ
    ICM40609_GYRO_NF_BW_20HZ   = 6,     //ICM40608_GYRO_NF_BW_5HZ
    ICM40609_GYRO_NF_BW_10HZ   = 7,     //ICM40608_GYRO_NF_BW_3HZ
} icm40609GyroNfBw_e;
...
static void icm40609GetAafParams(uint16_t targetHz, ICM40609_AafProfile* res)
{
    ...
    uint16_t i = 0;
    while (i < ICM40609_AAF_PROFILE_COUNT && targetHz >  aafProfiles_ICM40608[i].hz) {
         i++;
    }
    if (i < ICM40609_AAF_PROFILE_COUNT) {  
        *res = aafProfiles_ICM40608[i];
    } else {
        // not found - Requested frequency is higher than max available
        *res = aafProfiles_ICM40608[ICM40609_AAF_PROFILE_COUNT - 1]; 
    }
#else    
    uint16_t i = 0;
    ...
    if (notchEnable) {
...
#if defined(USE_ICM40608_AS_ICM40609D)         
        float coswz = cosf(2.0f * M_PIf * fdesiredKhz / 8.0f);
#else
        ...
        if (fabsf(coswz) <= 0.875f) {
            ...
            if (coswz > 0.875f) {
                ...
                nf_coswz = (int16_t)roundf(-8.0f * (1.0f + coswz) * 256.0f); //这里不管哪个手册均为-8
            }
            nf_coswz_lsb = nf_coswz & 0xFF;
            nf_coswz_msb = (nf_coswz >> 8) & 0x01;
        }
...
static void icm40609SetTempFiltBw(const extDevice_t *dev, icm40609TempFiltBw_e bw)   //两者相同
{
    icm40609SelectUserBank(dev, ICM40609_USER_BANK_0);
...
void icm40609GyroInit(gyroDev_t *gyro)
{
...
#if defined(USE_ICM40608_AS_ICM40609D)
    spiWriteReg(dev, ICM40609_REG_ACCEL_CONFIG0, ICM40608_ACCEL_FS_SEL_16G | ICM40609_ACCEL_ODR_1KHZ );
#else    
    ...
    spiWriteReg(&gyro->dev, ICM40609_REG_GYRO_ACCEL_CONFIG0, 
                    ...
                    ICM40609_GYRO_UI_FILT_BW_LP_TRIVIAL_200HZ_8XODR);  // 40608_9相同
    ...
    icm40609SetGyroNotch(dev, true, ICM40609_GYRO_NF_BW_1449HZ, 1.5f);  //ICM40608_GYRO_NF_BW_362HZ

    icm40609SetGyroHPF(dev, true, ICM40609_HPF_BW_1, ICM40609_HPF_ORDER_1ST);  //ICM40608/9均未发现该该滤波器具体定义
...
uint8_t icm40609SpiDetect(const extDevice_t *dev)
{
    ...
    if ((whoAmI == ICM40609_WHO_AM_I_CONST)||(whoAmI==ICM40608_WHO_AM_I_CONST)) {
