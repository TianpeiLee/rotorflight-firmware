...
void spiInternalResetDescriptors(busDevice_t *bus)
{
    ...
    initTx->DMA_PeripheralBaseAddr = (uint32_t)&bus->busType_u.spi.instance->DATAR;
    ...
    if (bus->dmaRx) {
        ...
        initRx->DMA_PeripheralBaseAddr = (uint32_t)&bus->busType_u.spi.instance->DATAR;
...
static bool spiInternalReadWriteBufPolled(SPI_TypeDef *instance, const uint8_t *txData, uint8_t *rxData, int len)
{
    ...
    while (len--) {
        ...
        // === 临时:加超时,避免 SPI 无响应时死等卡死 ===
        uint32_t timeout;
        timeout = 100000;
        while (SPI_I2S_GetFlagStatus(instance, SPI_I2S_FLAG_TXE) == RESET && timeout) timeout--;
        SPI_I2S_SendData(instance, b);

        timeout = 100000;
        while (SPI_I2S_GetFlagStatus(instance, SPI_I2S_FLAG_RXNE) == RESET && timeout) timeout--;
        // === 超时结束 ===
...
void spiInternalStartDMA(const extDevice_t *dev)
{
    ...
    if (dmaRx) {
        ...
        DMA_Cmd(streamRegsTx, DISABLE);
        DMA_Cmd(streamRegsRx, DISABLE);
...
void spiSequenceStart(const extDevice_t *dev)
{
    ...
    if (dev->busType_u.spi.leadingEdge != bus->busType_u.spi.leadingEdge) {
        ...
        instance->CTLR1 &= ~(SPI_CPOL_High | SPI_CPHA_2Edge);

        // Apply setting
        if (dev->busType_u.spi.leadingEdge) {
            instance->CTLR1 |= SPI_CPOL_Low | SPI_CPHA_1Edge;
        } else
        {
            instance->CTLR1 |= SPI_CPOL_High | SPI_CPHA_2Edge;
        }
