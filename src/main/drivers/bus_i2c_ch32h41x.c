...
void i2c_ev_handler(I2CDevice device) {
    ...
    if (SReg_1 & I2C_STAR1_SB) {                                                  // we just sent a start - EV5 in ref manual
        ...
        asm("fence");                                                               // memory fence to control hardware
        ...
        if (state->reading && subaddress_sent) {                         // EV7_2, EV7_3
            if (state->bytes > 2) {                                      // EV7_2
                I2C_AcknowledgeConfig(I2Cx, DISABLE);                   // turn off ACK
                state->read_p[index++] = (uint8_t)I2Cx->DATAR;              // read data N-2
                I2C_GenerateSTOP(I2Cx, ENABLE);                         // program the Stop
                final_stop = 1;                                         // required to fix hardware
                state->read_p[index++] = (uint8_t)I2Cx->DATAR;              // read data N - 1
                ...
                state->read_p[index++] = (uint8_t)I2Cx->DATAR;                    // read data N - 1
                state->read_p[index++] = (uint8_t)I2Cx->DATAR;                    // read data N
        ...
        state->read_p[index++] = (uint8_t)I2Cx->DATAR;
        ...
        if (index != -1) {                                              // we dont have a subaddress to send
            I2Cx->DATAR = state->write_p[index++];
